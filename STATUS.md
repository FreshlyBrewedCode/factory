...
>
> **The release leg (2026-09-15, stacked on the phase-5 PR) made the POC installable.** Phase 5
> left it usable only from inside this repo — every config and workflow imported relative paths
> into `src/`. Now the package has one declared public barrel (`src/index.ts` behind an `exports`
> map, D35), the repo imports through the same `@frebreco/factory` specifier a user types, and
> `factory init` scaffolds `.factory/` with a config and a starter workflow (D36). ADR 0008; the
> README is now user-facing. No runtime behaviour changed.
>
...
`bun test` is 145 green and the playwright suite is 11 specs through `nix develop`.

**The POC is installable, not just runnable here.** The release leg (D35/D36, ADR 0008) gave the
package a declared public surface and `factory init`; `bun run build --smoke-test` proves the
staged package resolves `@frebreco/factory` from a temp `node_modules`, scaffolds a project and
imports the config it generated.
...
| `src/cli.ts`, `cli.test.ts`, `cli.start.test.ts`, `cli.crash.test.ts`    | `factory run <workflow.ts>` (now also appends to sqlite), `factory start <workflowId>` (D31's thin HTTP client: prints the runId, `--watch` tails SSE), `factory runs`, `factory log <runId>`, and — from the release leg — `factory init` (D36) plus `--help`/no-args usage. `factory serve` auto-discovers the config when no `--config` is given                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | keep — phase 3's server wraps `run`, `start` is its explicit client |
...
| `src/index.ts`, `src/index.test.ts`                                      | **The public barrel (D35, ADR 0008)** — the only entry `package.json`'s `exports` map exposes: `defineWorkflow`, `defineConfig`, `Schema`, `isTerminal`, the `DEFAULT_*` constants, and the types a workflow body touches. Deep imports into `runtime/`/`server/`/`persistence/`/`web/` are unreachable. The test imports through the `@frebreco/factory` specifier (self-reference) and pins the exported key list, so widening the surface is a visible diff. Phase 0's `slugify` smoke test is gone                                                                                                                                                                                                                                                                                                                                                                                                                | keep — the contract                                                 |
| `src/init.ts`, `src/init.test.ts`                                        | `factory init` (D36): scaffolds `.factory/factory.config.ts` + `.factory/workflows/hello.ts`, fills the `repo` block from the checkout's `origin`/identity/default branch (flagged placeholders outside a git repo), and rewrites `.gitignore` to ignore only the regenerable half — **including stripping a pre-existing blanket `.factory/` line**. Never overwrites without `--force`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | keep                                                                |
| `sample/`                                                                | The phase-5 sample project, now importing `@frebreco/factory` like real user code. Deliberately keeps the **root-config layout**, so it doubles as `findFactoryConfig`'s fallback regression                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | keep                                                                |
| `bin/factory.js`, `scripts/build-release.ts`                             | ADR 0005's launcher and the release staging. The staging now mirrors the `exports` map into the generated manifest, drops `workflows/` from the package, and its `--smoke-test` installs the staged package into a temp `node_modules`, runs `factory init` there and imports the generated config                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | keep                                                                |
...
| D35 | **One public barrel behind an `exports` map**; the repo imports its own published specifier      | ADR 0008                            |
| D36 | **`.factory/` holds the project** — config + workflows committed, state ignored; `factory init` | ADR 0008, widens D27                |
...
**If you are looking at this as a user rather than a maintainer, read [`README.md`](README.md)
instead** — it is the user-facing document as of the release leg (quick start, the `ctx` surface,
the config keys, the CLI, and an honest "before you run it"). ADR 0008 is the maintainer's
counterpart: what the package exposes and why `.factory/` holds both source and state.
