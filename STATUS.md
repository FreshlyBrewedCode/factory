...
|                   |                                                                                                                                                                          |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Conclusions       | `docs/adr/0001-write-back-isolation-effect-boundary.md` (phase 0) · `0002-workflow-authoring-surface.md` (authoring syntax) · `0003-run-event-type.md` (D3's event type) · `0004-server-dispatch.md` (D22–D24, phase 3) · `0005-poc-manual-runs.md` (D27–D33, phase 5 — **Proposed**, pre-implementation) |
...
session — pre-implementation, the only decisions here that nothing has executed yet.**
...
| D27 | **`factory.config.ts` is the project's entry point**, with workflows imported into it | ADR 0005 |
| D28 | **One working tree per run**, cloned from a local bare mirror; last N retained | ADR 0005 |
| D29 | **One `maxConcurrentRuns`** behind a single admission function; 409 over the limit | ADR 0005 |
| D30 | **`GET /api/workflows`** served from the config's array, schemas as JSON Schema | ADR 0005 |
| D31 | **`POST /api/runs {workflowId, input}`**; `factory start` is a thin HTTP client | ADR 0005 |
| D32 | **Write-back metadata is agent-supplied**, branch included; collisions resolved reactively | ADR 0005 |
| D33 | **The start form is single-depth**, with a raw-JSON escape hatch | ADR 0005 |
...
`POST /api/runs/:id/cancel` has existed since phase 3 and is wired to nothing in `src/web/`.
_Validated by:_ playwright against the slow fake adapter — open the dialog, fill it, start, watch
it stream, cancel it.
...
Isolation (docker + secrets, D7's deferral — note D28 gave phase 5 *concurrency* without it, and
