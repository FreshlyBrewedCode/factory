# 0a-1 — Single agent step

Covers the 0a-1 spike subtask: one opencode agent step through `@tanstack/ai` +
`localProcessSandbox`, exercised against a real host clone. Ran 2026-09-13, commit `b752f0b`.
Headline result: the agent correctly implemented and tested `slugify` end to end, and reading
the installed `@tanstack/ai*` packages resolved most of phase 0's open questions about
workspace semantics, CUSTOM events, structured output, and process cleanup — see below for
...
dumped to NDJSON (D14). Not the full round trip (that's [0a-2](./0a-2-round-trip.md)).
...
means callers ([0a-2](./0a-2-round-trip.md), the PR-metadata step) should read that CUSTOM event
...
would accidentally stage it. For [0a-2](./0a-2-round-trip.md), either avoid `defineWorkspace(...)`
...
### 7. Stream shape (for [0b](./0b-effect-boundary.md)'s future Effect Stream wrapping)
...
with no circular refs or non-serializable values, which matters for [0b](./0b-effect-boundary.md)'s
